...
def run_market(market, tech, history_path: Path, forward_path: Path, years, degradation_map,
    ...
    price_matrix = summarize_prices(dist, w_da=w_da, w_rt=w_rt, quantiles=quantiles)
    price_matrix.to_csv(outdir / "fixed_price_matrix_P50_P75_P90.csv", index=False)

    drivers = volatility_driver_summaries(hourly)
    drivers.to_csv(outdir / "volatility_driver_summaries.csv", index=False)

    dist.to_csv(outdir / "scenario_unit_PV_distribution.csv", index=False)

    export_market_excel(
        market=market, outdir=outdir, price_matrix=price_matrix, gen_fore=gen_fore,
        drivers=drivers, dist=dist, forwards_long=fwd, wacc=wacc, w_da=w_da, w_rt=w_rt, years=years
    )

    return price_matrix, gen_fore, drivers
...
def main():
    ...
    for mkt, fwd in markets.items():
        outdir = OUTDIR / mkt
        print(f"\n--- Running {mkt} ({TECH_TYPES[mkt]}) ---")
        price_matrix, gen_fore, drivers = run_market(
            market=mkt, tech=TECH_TYPES[mkt],
            history_path=HISTORICAL_XLSX, forward_path=fwd,
...
# --- Master workbook aggregating all markets ---
...
with pd.ExcelWriter(master, engine="xlsxwriter") as w:
    for mkt in ["ERCOT","MISO","CAISO"]:
        base = OUTDIR / mkt
        # bring in the key CSVs you already produced
        pm   = pd.read_csv(base / "fixed_price_matrix_P50_P75_P90.csv")
        gen  = pd.read_csv(base / "expected_generation_2026_2030.csv")
        drv  = pd.read_csv(base / "volatility_driver_summaries.csv")
        scen = pd.read_csv(base / "scenario_unit_PV_distribution.csv")

        pm.to_excel(w, sheet_name=f"{mkt}_FairPrices", index=False)
        gen.to_excel(w, sheet_name=f"{mkt}_ExpectedGen", index=False)
        drv.to_excel(w, sheet_name=f"{mkt}_VolDrivers", index=False)
        scen.to_excel(w, sheet_name=f"{mkt}_Scenarios", index=False)
print(f"✓ Master Excel written: {(OUTDIR / 'Valuation_Report_ALL_MARKETS.xlsx').resolve()}")
